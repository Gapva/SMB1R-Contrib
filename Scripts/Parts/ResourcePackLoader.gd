...
func open_folder() -> void:
	OS.shell_show_in_file_manager(ProjectSettings.globalize_path(Global.data_dir + "resource_packs"), true)
...
func get_resource_packs() -> void:
	...
	containers = []
	resource_packs = []
	for i in DirAccess.get_directories_at(Global.data_dir + "resource_packs"):
		resource_packs.append(i)
	for i in resource_packs:
		var pack_info_path = Global.data_dir + "resource_packs/" + i + "/" + "pack_info.json"
		if FileAccess.file_exists(pack_info_path) and i != Global.ROM_PACK_NAME:
			create_container(Global.data_dir + "resource_packs/" + i)
...
func create_container(resource_pack := "") -> void:
	...
	container.pack_name = resource_pack.replace(Global.data_dir + "resource_packs/", "")
