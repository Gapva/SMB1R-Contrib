...
func save_recording() -> void:
	...
	var recording_dir = Global.data_dir + "marathon_recordings/" + Global.current_campaign
...
func load_recording(world_num := 0, level_num := 0, is_warpless := true, campaign := "SMB1") -> Array:
	var recording_dir = Global.data_dir + "marathon_recordings/" + campaign
...
func load_best_times(campaign = Global.current_campaign) -> void:
	...
	for world_num in 8:
		for level_num in 4:
			var path = Global.data_dir + "marathon_recordings/" + campaign + "/" + str(world_num + 1) + "-" + str(level_num + 1) + ".json"
			...
			path = Global.data_dir + "marathon_recordings/" + campaign + "/" + str(world_num + 1) + "-" + str(level_num + 1) +"warp" + ".json"
			if FileAccess.file_exists(path):
