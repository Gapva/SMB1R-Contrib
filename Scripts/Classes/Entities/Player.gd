...
func apply_character_physics() -> void:
	...
	if int(Global.player_characters[player_id]) > 3:
		path = path.replace("res://Assets/Sprites/Players", Global.data_dir + "custom_characters")
...
func apply_character_sfx_map() -> void:
	...
	for i in json:
		...
		if FileAccess.file_exists(res_path) == false or custom_character:
			...
			if int(Global.player_characters[player_id]) > 3:
				directory = directory.replace("res://Assets/Sprites/Players", Global.data_dir + "custom_characters")
